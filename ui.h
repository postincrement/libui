...
extern "C" {
 ...
 /*! \mainpage libui: a portable GUI library for C
  *
  * Read the [README](README.md)
  *
  * Read [API documentation](modules.html)
  *
  */
...
/**
  ...
  *  \param err error string returned by uiInit
  *  \return none
  *  \see uiInit
  */
...
/** \defgroup uiControl uiControl
  * \brief The top level GUI control that provdes methods common to all uiControls.
  *
  *  @{
  */
...
/** \def uiControl
  * \brief cast the pointer to a uiControl type
  *
  * \param this value to cast
  * \return uiControl *
  */
#define uiControl(this) ((uiControl *) (this))

/** \brief destroys the Control
  * \param ctl control to use
  * \return none
  */
_UI_EXTERN void uiControlDestroy(uiControl * ctl);

/** \brief Handle returns the OS-level handle that backs the Control. 
  *
  * On OSs that use reference counting for
  * controls, uiControlHandle does not increment the reference
  * count; you are sharing package ui's reference.
  *
  * \param ctl control to use
  * \return handle cast to a uintptr_t
  */
_UI_EXTERN uintptr_t uiControlHandle(uiControl * ctl);
...
/** \brief returns whether the Control is visible.
  *
  * \param ctl control to use
  * \return non-zero if control is visible, else zero
  * \see uiControlShow
  * \see uiControlHide
  */
_UI_EXTERN int uiControlVisible(uiControl * ctl);

/** \brief Show the Control.
  *
  * Opposite of uiHideControl
  *
  * \param ctl control to use
  * \return none
  * \see uiControlHide
  * \see uiControlVisible
  */
_UI_EXTERN void uiControlShow(uiControl * ctl);

/** \brief Hide the Control.
  *
  * Hidden controls do not participate in layout (that is, 
  * Box, Grid, etc. does not reserve space for hidden controls
  *
  * \param ctl control to use
  * \return none
  * \see uiControlShow
  * \see uiControlVisible
  */
_UI_EXTERN void uiControlHide(uiControl * ctl);

/** \brief returns whether the Control is enabled.
  *
  * \param ctl control to use
  * \return non-zero if control is enabled, else zero
  * \see uiControlEnable
  * \see uiControlDisable
  */
_UI_EXTERN int uiControlEnabled(uiControl * ctl);
...
/** \brief Enable the Control.
  ...
  */
_UI_EXTERN void uiControlEnable(uiControl * ctl);
...
/** \brief Disable the Control.
  ...
  * \see uiControlEnabled
  */
_UI_EXTERN void uiControlDisable(uiControl * ctl);
...
/**
  * \defgroup uiWindow uiWindow
  * \brief A uiControl that represents a top-level window.
  *
...
/**
  * \defgroup uiButton uiButton
  * \brief A uiControl that represents a button that the user can click to perform an action. 
  *
...
/**
  * \defgroup uiBox uiBox
  * \brief A uiControl that holds a group of uiControls horizontally or vertically. 
  * 
  ...
  * If multiple controls are marked stretchy, they will be given equal shares of the leftover space. 
...
/**
  * \defgroup uiCheckBox uiCheckBox
  * \brief A uiControl that represents a box with a text label at its side. 
  *
  * When the user clicks the checkbox, a check mark will appear in the box; clicking it again removes the check. 
  *
  *  @{
  */
...
/**
  * \defgroup uiEntry uiEntry
  * \brief A uiControl that represents a space that the user can type a single line of text into.
  *
  *  @{
  */
...
/**
  * \defgroup uiArea uiArea
  * \brief Represents a blank canvas that a program can draw on as it wishes. 
  *
  * uiAreas also receive keyboard and mouse events, and programs can react to those as they see fit. 
  * Drawing and event handling are handled through an instance of a type that implements uiAreaHandler that every 
  * uiArea has; see uiAreaHandler for details.
  *
  * There are two types of areas. Non-scrolling areas are rectangular and have no scrollbars. 
  * Programs can draw on and get mouse events from any point in the uiArea, and the size of the uiArea is decided by package ui itself, 
  * according to the layout of controls in the uiWindow the uiArea is located in and the size of said uiWindow. There is no way to query the 
  * Area's size or be notified when its size changes; instead, you are given the area size as part of the draw and mouse event handlers, 
  * for use solely within those handlers.
  * 
  * Scrolling areas have horziontal and vertical scrollbars. The amount that can be scrolled is determined by the area's size, 
  * which is decided by the programmer (both when creating the uiArea and by a call to SetSize). Only a portion of the uiArea is visible at 
  * any time; drawing and mouse events are automatically adjusted to match what portion is visible, so you do not have to worry about 
  * scrolling in your event handlers. uiAreaHandler has more information.
  *
  * The internal coordinate system of a uiArea is points, which are floating-point and device-independent. 
  * For more details, see uiAreaHandler. The size of a scrolling Area must be an exact integer number of points 
  * (that is, you cannot have an Area that is 32.5 points tall) and thus the parameters to NewScrollingArea and SetSize are 
  * ints. All other instances of points in parameters and structures (including sizes of drawn objects) are float64s.
  *
  *  @{
  */

/**
  * \struct uiAreaHandler uiAreaHandler
  * \brief Defines the functionality needed for handling events from an uiArea. 
  *
  * Each of the methods on uiAreaHandler is called from the GUI thread, and every 
  * parameter (other than the uiArea itself) should be assumed to only be valid during 
  * the life of the method call (so for instance, do not save uiAreaDrawParams.AreaWidth, 
  * as that might change without generating an event).
  *
  * Coordinates to Draw and MouseEvent are given in points. 
  * Points are generic, floating-point, device-independent coordinates with (0,0) 
  * at the top left corner. You never have to worry about the mapping between points and
  * pixels; simply draw everything using points and you get nice effects like looking
  * sharp on high-DPI monitors for free. Proper documentation on the matter is 
  * being written. In the meantime, there are several referenes to this kind of 
  ...
  *
  * For a scrolling Area, points are automatically offset by the scroll position. 
  * So if the mouse moves to position (5,5) while the horizontal scrollbar is *
  * at position 10 and the horizontal scrollbar is at position 20, the coordinate 
  * stored in the AreaMouseEvent structure is (15,25). The same applies to drawing.
  *
  */
